<?php
session_start();
if(!isset($_SESSION['managerId'])){ header('location:login.php');}
?>
<!DOCTYPE html>
<html>
<head>
  <title>Banking</title>
  <?php require 'assets/autoloader.php'; ?>
  <?php require 'assets/db.php'; ?>
  <?php require 'assets/function.php'; ?>
  <?php if (isset($_GET['delete'])) 
  {
    if ($con->query("delete from useraccounts where id = '$_GET[id]'"))
    {
      header("location:mindex.php");
    }
  } ?>
</head>
<body style="background: url('images/maddaccount-bg.jpg') no-repeat center center; background-size: cover;">
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
 <a class="navbar-brand" href="#">
    <img src="images/logo.png" width="30" height="30" class="d-inline-block align-top" alt="">
   <!--  <i class="d-inline-block  fa fa-building fa-fw"></i> --><?php echo bankName; ?>
  </a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item ">
        <a class="nav-link active" href="mindex.php">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item ">  <a class="nav-link" href="maccounts.php">Accounts</a></li>
      <li class="nav-item ">  <a class="nav-link" href="maddnew.php">Add New Account</a></li>
      <li class="nav-item ">  <a class="nav-link" href="mfeedback.php">Feedback</a></li>
      <!-- <li class="nav-item ">  <a class="nav-link" href="transfer.php">Funds Transfer</a></li>
       <li class="nav-item ">  <a class="nav-link" href="profile.php">Profile</a></li> -->


    </ul>
    <?php include 'msideButton.php'; ?>
    
  </div>
</nav><br><br><br>
<?php 
  $array = $con->query("select * from useraccounts,branch where useraccounts.id = '$_GET[id]' AND useraccounts.branch = branch.branchId");
  $row = $array->fetch_assoc();
  $userId = $row['id'];
  $query = $con->query("SELECT * FROM transaction WHERE userId = '$userId' ORDER BY date DESC");
 ?>
<div class="container">
<div class="card w-100 text-center shadowBlue">
  <div class="card-header">
    Account profile for <?php echo $row['name'];echo "<kbd>#";echo $row['accountNo'];echo "</kbd>"; ?>
  </div>
  <div class="card-body">
    <table class="table table-bordered">
      <tbody>
        <tr>
          <td>Name</td>
          <th><?php echo $row['name'] ?></th>
          <td>Account No</td>
          <th><?php echo $row['accountNo'] ?></th>
        </tr><tr>
          <td>Branch Name</td>
          <th><?php echo $row['branchName'] ?></th>
          <td>Branch Code</td>
          <th><?php echo $row['branchNo'] ?></th>
        </tr><tr>
          <td>Current Balance</td>
          <th> ৳ <?php echo $row['balance'] ?></th>
          <td>Account Type</td>
          <th><?php echo $row['accountType'] ?></th>
        </tr><tr>
          <td>NID</td>
          <th><?php echo $row['nid'] ?></th>
          <td>City</td>
          <th><?php echo $row['city'] ?></th>
        </tr><tr>
          <td>Contact Number</td>
          <th><?php echo $row['number'] ?></th>
          <td>Address</td>
          <th><?php echo $row['address'] ?></th>
        </tr>
      </tbody>
    </table>
    <h4 class="text-center mt-4">Transaction Summary</h4>
<div class="card-body">
<?php 
  $accountNo = $row['accountNo'];
  // $query = $con->query("SELECT * FROM transaction WHERE accountNo = '$accountNo' ORDER BY date DESC");
  $query = $con->query("SELECT * FROM transaction WHERE userId = '$userId' ORDER BY date DESC");

  if ($query->num_rows > 0) {
    while ($txn = $query->fetch_assoc()) {
      if ($txn['action'] == 'withdraw') {
  echo "<div class='alert alert-secondary'>Account withdrawal BDT. {$txn['debit']} at {$txn['date']}</div>";
}
if ($txn['action'] == 'deposit') {
  echo "<div class='alert alert-success'>Account deposit BDT. {$txn['credit']} at {$txn['date']}</div>";
}
if ($txn['action'] == 'transfer') {
  echo "<div class='alert alert-warning'>Account transfer BDT. {$txn['debit']} at {$txn['date']} to account no. {$txn['other']}</div>";
}
    }
  } else {
    echo "<div class='alert alert-info'>No transactions found for this account.</div>";
  }
?>
</div>
  </div>
  <div class="card-footer text-muted">
    <?php echo bankName; ?>
  </div>
</div>
<footer class="bg-dark text-white text-center py-3">
  <div class="container">
    <p><strong>MCB Bank</strong> — Secure. Reliable. Trusted.</p>
    <p>
      <a href="mindex.php" class="text-white mx-2">Home</a>
      <a href="maccounts.php" class="text-white mx-2">Accounts</a>
      <a href="maddnew.php" class="text-white mx-2">Add Account</a>
      <a href="mfeedback.php" class="text-white mx-2">Feedback</a>
    </p>
    <p>Email: support@mcbank.com | Phone: +880 1234-567890</p>
    <p>&copy; <?php echo date("Y"); ?> MCB Bank. All Rights Reserved.</p>
  </div>
</footer>

</body>
</html>