...
if (in_array($_SERVER['SERVER_NAME'], ['localhost', '127.0.0.1'])) {
	...
	if (isset($_POST['managerLogin'])) {
		$user = $_POST['email'];
		$pass = md5($_POST['password']); // ✅ Hash password (important!)
		
		// ✅ Use correct table (managers)
		$result = $con->query("SELECT * FROM managers WHERE email='$user' AND password='$pass'");
		
		if ($result->num_rows > 0) { 
			session_start();
			$data = $result->fetch_assoc();
			$_SESSION['managerId'] = $data['id'];
			$_SESSION['managerName'] = $data['name']; // optional
			header('location:mindex.php');
			exit();
		} else {
			$error = "<div class='alert alert-warning text-center rounded-0'>Manager username or password wrong, try again!</div>";
		}
	}
	 ?>
</head>
...
<body style="background: url(images/bg-login4.jpg); background-size: cover; min-height:100vh; display:flex; flex-direction:column;">
...
<!-- <main class="flex-fill d-flex justify-content-end align-items-center"> -->
  ...
  <!-- <div id="accordion" role="tablist" class="w-25 shadowBlack me-5"> -->
    ...
    <!-- ✅ FIXED MANAGER LOGIN -->
		<div class="card rounded-0">
			<div class="card-header" role="tab" id="headingTwo">
				<h5 class="mb-0">
					<a class="collapsed btn btn-outline-success btn-block" data-toggle="collapse" href="#collapseTwo" aria-expanded="false">
						Manager Login
					</a>
				</h5>
			</div>
    ...
    <div class="text-center mt-4 mb-3">
       <p class="text-white">Don’t have an account? <a href="chooseSignup.php" class="text-warning font-weight-normal">Register now</a></p>
...
<!-- <footer class="bg-success text-white pt-5 pb-3 mt-auto"> -->
  <footer class="bg-success text-white pt-3 mt-auto" style="background-color: #146c43;">
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-2 text-sm">
        ...
        <ul class="list-unstyled">
          ...
          <li><a href="chooseSignup.php" class="text-white text-decoration-none">Sign Up</a></li>
        </ul>
      </div>
